...
import {
	sendPassageDataToClient,
	toUpdatePassage,
	updatePassages,
	focusPassage,
} from "./story-map/socket";
import { startUI, stopUI, storyMapIO } from "./story-map";

import { fileGlob } from "./file-ops";

import {
	PassageSymbolProvider,
	PassageListProvider,
	Passage,
	jumpToPassage,
	WorkspacePassageSymbolProvider,
	passageAtCursor,
} from "./passage";
...
export const PackageLanguages = PackageContributions.languages.map(
	(el) => el.id
);
...
export async function activate(ctx: vscode.ExtensionContext) {
	...
	async function prepare() {
		...
		await Promise.all(allPassages)
			.then((passages) => {
				return ctx.workspaceState.update(
					"passages",
					passages.reduce((acc, val) => acc.concat(val), [] as Passage[])
				);
			})
			.then(() => {
				...
				if (
					vscode.workspace
						.getConfiguration("twee3LanguageTools.passage")
						.get("list")
				)
	...
	const mapShowCommand = vscode.commands.registerCommand(
		"twee3LanguageTools.storyMap.show",
		startUIWrapper
	);
	const mapStopCommand = vscode.commands.registerCommand(
		"twee3LanguageTools.storyMap.stop",
		stopUIWrapper
	);

	if (
		vscode.workspace
			.getConfiguration("twee3LanguageTools.storyMap")
			.get("restoreStoryMapOnLaunch") &&
	...
	ctx.subscriptions.push(
		...
		vscode.languages.registerDocumentSemanticTokensProvider(
			documentSelector,
			new DocumentSemanticTokensProvider(ctx),
			legend
		),
		vscode.languages.registerDocumentSymbolProvider(
			documentSelector,
			new PassageSymbolProvider(ctx)
		),
		vscode.languages.registerWorkspaceSymbolProvider(
			new WorkspacePassageSymbolProvider(ctx)
		),
		vscode.languages.registerHoverProvider(documentSelector, {
			provideHover(
				...
				switch (document.languageId) {
					case sugarcube2Language.LanguageID:
						return sugarcube2Macros.hover(document, position, token);
					default:
						return null;
				}
			},
		...
		vscode.languages.registerDefinitionProvider(configurationSelector, {
			provideDefinition(
				...
				return sugarcube2Macros.definitionConfig(
					ctx,
					document,
					position,
					token
		...
		vscode.window.onDidChangeTextEditorSelection(async (e) => {
			if (
				e.textEditor.document.languageId === sugarcube2Language.LanguageID &&
				vscode.workspace
					.getConfiguration("twee3LanguageTools.sugarcube-2.features")
					.get("macroTagMatching")
			) {
				let collected = await sugarcube2Macros.collectCache.get(
				...
				e.selections.forEach((sel) => {
					...
					let target = collected.macros
						.filter((el) => el.open && el.id !== el.pair)
						.reverse()
						.find((el) =>
							new vscode.Range(
								el.range.start,
								collected.macros[el.pair].range.end
							).contains(pos)
						);
					if (target) {
						r.push(
		...
		vscode.workspace.onDidChangeConfiguration((e) => {
			...
			if (
				e.affectsConfiguration(
					"twee3LanguageTools.sugarcube-2.cache.argumentInformation"
				) &&
				!vscode.workspace
		...
		vscode.workspace.onDidDeleteFiles((e) => {
			e.files.forEach((file) =>
				sugarcube2Macros.collectCache.clearFilename(file.fsPath)
			);

			const removedFilePaths = e.files.map((file) => file.path);
			const oldPassages: Passage[] = ctx.workspaceState.get("passages", []);
			const newPassages: Passage[] = oldPassages.filter(
				(passage) => !removedFilePaths.includes(passage.origin.full)
			);
			...
			for (let file of e.files) {
				...
				passages.forEach((el) => {
					if (el.origin.full === file.oldUri.path) {
						el.origin.root =
							vscode.workspace.getWorkspaceFolder(file.newUri)?.uri.path || "";
		...
		vscode.commands.registerCommand(
			"twee3LanguageTools.refreshDiagnostics",
			() => {
		...
		vscode.commands.registerCommand(
			"twee3LanguageTools.passage.pack",
			async () => {
				...
				if (proceed === "Proceed") {
					...
					updatePassages(
						ctx,
						packer(passages).map((p: Passage) => toUpdatePassage(p))
					);
				}
			}
		),
		vscode.commands.registerCommand(
			"twee3LanguageTools.passage.jump",
			(item: Passage) => {
				jumpToPassage(item);
			}
		),
		vscode.commands.registerCommand("twee3LanguageTools.passage.list", () => {
			const config = vscode.workspace.getConfiguration(
				"twee3LanguageTools.passage"
			);
			config.update("list", !config.get("list"));
		}),
		vscode.commands.registerCommand(
			"twee3LanguageTools.passage.group.none",
			() => {
				vscode.workspace
					.getConfiguration("twee3LanguageTools.passage")
					.update("group", "None");
		...
		vscode.commands.registerCommand(
			"twee3LanguageTools.passage.group.file",
			() => {
				vscode.workspace
					.getConfiguration("twee3LanguageTools.passage")
					.update("group", "File");
		...
		vscode.commands.registerCommand(
			"twee3LanguageTools.passage.group.folder",
			() => {
				vscode.workspace
					.getConfiguration("twee3LanguageTools.passage")
					.update("group", "Folder");
		...
		vscode.commands.registerCommand(
			"twee3LanguageTools.passage.group.tag",
			() => {
				vscode.workspace
					.getConfiguration("twee3LanguageTools.passage")
					.update("group", "Tag");
		...
		vscode.commands.registerCommand("twee3LanguageTools.ifid.generate", () => {
			vscode.window.activeTextEditor?.insertSnippet(
				new vscode.SnippetString(uuidv4().toUpperCase())
			);
		}),
		vscode.commands.registerCommand(
			"twee3LanguageTools.sc2.defineMacro",
			sugarcube2CodeActions.unrecognizedMacroFixCommand
		),
		vscode.commands.registerCommand(
			"twee3LanguageTools.sc2.clearArgumentCache",
			() => {
		...
		vscode.commands.registerCommand(
			"twee3LanguageTools.sc2.addAllUnrecognizedMacros",
			() => {
				sugarcube2CodeActions.addAllUnrecognizedMacros();
			}
		),
		vscode.commands.registerCommand(
			"twee3LanguageTools.sc2.addAllUnrecognizedMacrosInFile",
			async () => {
				let editor = vscode.window.activeTextEditor;
				if (editor) {
					await sugarcube2CodeActions.addAllUnrecognizedMacrosInCurrentFile(
						editor.document
					);
				}
			}
		...
		vscode.commands.registerTextEditorCommand(
			"twee3LanguageTools.storyMap.focusPassage",
			(editor) => focusPassage(ctx, storyMap, editor)
		),
		vscode.commands.registerTextEditorCommand(
			"twee3LanguageTools.passage.setAsStart",
			async (editor) => {
				const passage = passageAtCursor(ctx, editor);
				if (passage) {
					const passages = ctx.workspaceState.get("passages", []) as Passage[];
					const storyData = passages.find(
						(passage) => passage.name === "StoryData"
					);
					if (storyData) {
						const range = new vscode.Range(
							storyData.range.start.translate(1),
							storyData.range.end
						);
						...
						edit.replace(
							vscode.Uri.file(storyData.origin.full),
							range,
							dataString
						);
		...
		vscode.commands.registerTextEditorCommand(
			"twee3LanguageTools.toggleItalics",
			(editor) => {
		...
		vscode.languages.registerCodeActionsProvider(
			sugarcube2Language.LanguageID,
			new sugarcube2CodeActions.EndMacro(),
			{
				providedCodeActionKinds:
					sugarcube2CodeActions.EndMacro.providedCodeActionKinds,
			}
		),
		vscode.languages.registerCodeActionsProvider(
			sugarcube2Language.LanguageID,
			new sugarcube2CodeActions.Unrecognized(),
			{
				providedCodeActionKinds:
					sugarcube2CodeActions.Unrecognized.providedCodeActionKinds,
			}
		),
		vscode.commands.registerCommand(
			"twee3LanguageTools.passageCounter.clickCheck",
			sbStoryMapConfirmationDialog
		)
	);
